...
# Google Compute Engine bootstrap for CPU-only OBLITERATUS hosting.
# Intended for Ubuntu images on Compute Engine.

APP_USER="${APP_USER:-obliteratus}"
APP_DIR="${APP_DIR:-/opt/obliteratus}"
APP_PORT="${APP_PORT:-8080}"
HOST_BIND="${HOST_BIND:-127.0.0.1}"
ENABLE_NGINX="${ENABLE_NGINX:-1}"
INSTALL_OPS_AGENT="${INSTALL_OPS_AGENT:-0}"
...
echo "Bootstrapping OBLITERATUS on Google Compute Engine"
...
if [[ -d "${APP_DIR}/.git" ]]; then
  ...
  echo "Using preloaded source tree at ${APP_DIR}"
else
...
sudo -u "${APP_USER}" "${APP_DIR}/.venv/bin/pip" install --index-url https://download.pytorch.org/whl/cpu "torch>=2.0"
